...
public class One710261947ReadWriteTextFiles {
    ...
    public static void WritingBackContactListToRecord(ArrayList<Contact> contactArrayList) {
        ...
        /*Path path = Paths.get("E:\\UEL2017_18\\CN7021AdavancedSoftwareEngineering\\CN7021CourseworkAdvancedSoftwareEngineeringCoursework\\src\\preparationAndLearning\\171026RegexReadWriteTextFiles\\java\\1710281553_4ContactData.txt");
        try { Files.lines(path).forEach(s->formFirstList(s));
        } catch (IOException e) {
            System.out.println(e);
        }
        contactArrayList=ConstructContactListFromRecord(firstList);
    ...
    static ArrayList<Contact> ConstructContactListFromRecord() {
        ...
        for(int i=0; i<firstList.size();i++) {
            ...
            //contactArrayList.get(i / 6).
            // setMeetings(new ArrayList<String>(Arrays.asList(firstList.get(i).split(" "))));
        }
        //temp for checking
        for (Contact c: contactArrayList)
        {
